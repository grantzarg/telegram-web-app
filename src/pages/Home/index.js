import React, {useLayoutEffect} from "react";
import { useNavigate } from "react-router-dom";
import css from './index.module.css'
import classnames from "classnames";
import Balance from "../../components/Balance";
import History from "../../components/History";

const Home = ({isAuthorized}) => {
    const navigate = useNavigate();

    const handleClick = () => {
        navigate('/deal');
    }

    useLayoutEffect(() => {
        if (!isAuthorized) {
            navigate('/auth', {replace: true})
            window.location.reload()
        }
    }, [isAuthorized])

    if (!isAuthorized) {
        return null
    }

    return (
        <div className={css.wrapper}>
            <Balance/>

            <div className={css.actionButtonsWrapper}>
                <div className={classnames(css.actionButton, css.actionButtonReceive)} onClick={() => navigate('/receive')}>
                    <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" transform="rotate(180)"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round" stroke="#FFFCCCCCC" stroke-width="1.056"></g><g id="SVGRepo_iconCarrier"> <path d="M12 5V19M12 5L6 11M12 5L18 11" stroke="#FFF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>                    <span>Пополнить</span>
                </div>
                <div className={classnames(css.actionButton, css.actionButtonSend)} onClick={() => navigate('/send')}>
                    <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M12 5V19M12 5L6 11M12 5L18 11" stroke="#FFF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                    <span>Вывести</span>
                </div>
            </div>

            <div className={css.dealButton} onClick={handleClick}>
                <svg viewBox="0 0 24 24" version="1.1" fill="#000000">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <title>transfer_3_line</title>
                        <g id="页面-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                            <g id="Arrow" transform="translate(-816.000000, 0.000000)" fill-rule="nonzero">
                                <g id="transfer_3_line" transform="translate(816.000000, 0.000000)">
                                    <path d="M24,0 L24,24 L0,24 L0,0 L24,0 Z M12.5934901,23.257841 L12.5819402,23.2595131 L12.5108777,23.2950439 L12.4918791,23.2987469 L12.4918791,23.2987469 L12.4767152,23.2950439 L12.4056548,23.2595131 C12.3958229,23.2563662 12.3870493,23.2590235 12.3821421,23.2649074 L12.3780323,23.275831 L12.360941,23.7031097 L12.3658947,23.7234994 L12.3769048,23.7357139 L12.4804777,23.8096931 L12.4953491,23.8136134 L12.4953491,23.8136134 L12.5071152,23.8096931 L12.6106902,23.7357139 L12.6232938,23.7196733 L12.6232938,23.7196733 L12.6266527,23.7031097 L12.609561,23.275831 C12.6075724,23.2657013 12.6010112,23.2592993 12.5934901,23.257841 L12.5934901,23.257841 Z M12.8583906,23.1452862 L12.8445485,23.1473072 L12.6598443,23.2396597 L12.6498822,23.2499052 L12.6498822,23.2499052 L12.6471943,23.2611114 L12.6650943,23.6906389 L12.6699349,23.7034178 L12.6699349,23.7034178 L12.678386,23.7104931 L12.8793402,23.8032389 C12.8914285,23.8068999 12.9022333,23.8029875 12.9078286,23.7952264 L12.9118235,23.7811639 L12.8776777,23.1665331 C12.8752882,23.1545897 12.8674102,23.1470016 12.8583906,23.1452862 L12.8583906,23.1452862 Z M12.1430473,23.1473072 C12.1332178,23.1423925 12.1221763,23.1452606 12.1156365,23.1525954 L12.1099173,23.1665331 L12.0757714,23.7811639 C12.0751323,23.7926639 12.0828099,23.8018602 12.0926481,23.8045676 L12.108256,23.8032389 L12.3092106,23.7104931 L12.3186497,23.7024347 L12.3186497,23.7024347 L12.3225043,23.6906389 L12.340401,23.2611114 L12.337245,23.2485176 L12.337245,23.2485176 L12.3277531,23.2396597 L12.1430473,23.1473072 Z" id="MingCute" fill-rule="nonzero"> </path>
                                    <path d="M8.20724,11.7572 C8.59776,12.1478 8.59776,12.7809 8.20724,13.1715 L6.37881,14.9999 L16.0001,14.9999 C16.5524,14.9999 17.0001,15.4476 17.0001,15.9999 C17.0001,16.5522 16.5524,16.9999 16.0001,16.9999 L6.37881,16.9999 L8.20724,18.8283 C8.59776,19.2188 8.59776,19.852 8.20724,20.2425 C7.81672,20.6331 7.18355,20.6331 6.79303,20.2425 L3.25749,16.707 C2.86697,16.3165 2.86697,15.6833 3.25749,15.2928 L6.79303,11.7572 C7.18355,11.3667 7.81672,11.3667 8.20724,11.7572 Z M15.7929,3.75725 C16.1534538,3.39676077 16.7206349,3.36903083 17.1128989,3.67406018 L17.2071,3.75725 L20.7427,7.29278 C21.1031615,7.65326923 21.1308893,8.22049645 20.8258834,8.61279152 L20.7427,8.707 L17.2071,12.2425 C16.8166,12.6331 16.1835,12.6331 15.7929,12.2425 C15.4324385,11.8820385 15.4047107,11.3147793 15.7097166,10.9225027 L15.7929,10.8283 L17.6214,8.99989 L8,8.99989 C7.44772,8.99989 7,8.55217 7,7.99989 C7,7.48704929 7.38604429,7.06438214 7.88337975,7.00661766 L8,6.99989 L17.6214,6.99989 L15.7929,5.17146 C15.4024,4.78094 15.4024,4.14777 15.7929,3.75725 Z" id="形状" fill="#FFF"> </path>
                                </g>
                            </g>
                        </g>
                    </g>
                </svg>
                <span>Быстрый перевод</span>
            </div>
            <History/>
        </div>
    )
}

export default Home
